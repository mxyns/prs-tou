...
void tou_write_packet(
    ...
    TOU_DEBUG(printf(TOU_DEFAULT_MSS >= header_size + size ? "true\n" : "false\n"));
...
int tou_recv_packet_parse(
    ...
    if ((pkt_type_ptr = tou_cbuffer_peek(conn->recv_work_buffer, 0)) == NULL) {
        TOU_DEBUG(printf("[tou][tou_recv_packet] CANT PEEK\n"));
        return 0;
    }
    char pkt_type = *pkt_type_ptr;
    TOU_DEBUG(printf("pkt_type %d\n", pkt_type));

    if (pkt_type == TOU_ID_DTP) {

        TOU_DEBUG(printf("type==dtp\n"));
        ...
        TOU_DEBUG(
                printf(
                        ...
                        TOU_SLL_ISEMPTY(conn->recv_window->list) ? -1
                                                                 : ((tou_packet_dtp*) conn->recv_window->list->head->val)->packet_id
                )
        );
        ...
        TOU_DEBUG(printf("[tou][tou_recv_packet] NOT SUPPOSED TO HAVE HANDSHAKE %d PACKET TYPE.", pkt_type));
        exit(1);
        return 0;

    } else {
        TOU_DEBUG(printf("[tou][tou_recv_packet] NOT SUPPOSED TO HAVE %d PACKET TYPE.", pkt_type));
        exit(1);
        return 0;
    }
}
...
// returns amount of new packets parsed
int tou_recv_packet(
        tou_conn* conn
) {
    ...
    TOU_DEBUG(printf("[tou][tou_recv_packet] read %d from socket\n", count));

    // TODO drop packets when full
    if (conn->recv_window->list->cap - conn->recv_window->list->count == 0) {
        TOU_DEBUG(printf("[tou][tou_recv_packet] recv_window is full\n"));
        return 0;
    }
    ...
    while (conn->recv_work_buffer->cnt > 0) {
        new += tou_recv_packet_parse(conn);
    }
...
void tou_send_2(
    ...
    while (size - written > 0) {
        ...
        tou_send(conn);
        TOU_DEBUG(printf("[tou][tou_send_2] buffer sent\n"));
...
int tou_send(
    ...
    while ((size = conn->out->cnt) > 0 && send_list->count < send_list->cap) {
        ...
        */
        ...
        if (popped <= 0) {
            TOU_DEBUG(printf("[tou][tou_send] no more data in out buffer to send\n"));
            return new_packets;
        }

        tou_packet_dtp* pkt = tou_sll_insert(send_list, id);
        if (pkt == NULL) {
            TOU_DEBUG(printf("[tou][tou_send] send window is full\n"));
            return new_packets;
        }
