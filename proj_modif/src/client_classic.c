...
int main(void) {
    ...
    while (1) {
        ...
        TOU_DEBUG(printf("Enter message: "));
        *fgets(client_message, 2000, stdin);
        ...
        if (sendto(mysock->fd, client_message, strlen(client_message), 0,
            ...
            TOU_DEBUG(printf("Unable to send message\n"));
            return -1;
        }
        ...
        if (recvfrom(mysock->fd, server_message, sizeof(server_message), 0,
                     mysock->peer_addr, &mysock->peer_addr_len) < 0) {
            TOU_DEBUG(printf("Error while receiving server's msg\n"));
            return -1;
        }

        TOU_DEBUG(printf("Server's response: %s\n", server_message));
    }
