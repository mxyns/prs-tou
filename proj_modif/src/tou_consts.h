#ifndef TOU_CONSTS_H
#define TOU_CONSTS_H

#ifdef SERVER1
#define TOU_CONFIG_NAME "SERVER_1"
#define TOU_DEFAULT_MSS (1500) // max payload size

#define TOU_DEFAULT_RECVWINDOW_SIZE (10)
#define TOU_DEFAULT_SENDWINDOW_SIZE (60)
#define TOU_DEFAULT_RECV_WORKBUFFER_SIZE (TOU_DEFAULT_MSS * 3)
#define TOU_DEFAULT_RECV_INBUFFER_SIZE (TOU_DEFAULT_MSS * 3)
#define TOU_DEFAULT_SEND_WORKBUFFER_SIZE (TOU_DEFAULT_MSS * 3)
#define TOU_DEFAULT_SEND_OUTBUFFER_SIZE (TOU_DEFAULT_MSS * TOU_DEFAULT_SENDWINDOW_SIZE)
#define TOU_DEFAULT_RECV_CTRLBUFFER_SIZE ((TOU_LEN_PKT_ACK + sizeof(ack_id_t) * TOU_PKT_ACK_MAX_ACK_COUNT) * 5)
#define TOU_DEFAULT_ACK_TIMEOUT_MS (3)
#define TOU_DEFAULT_FAST_RETRANSMIT_ACK_COUNT (3)
#define TOU_DEFAULT_RETRANSMIT_METHOD(conn, expired_pkt, dropped_id, ack_stack_count) tou_retransmit_all(conn)
#endif

#ifdef SERVER2
#define TOU_CONFIG_NAME "SERVER_2"
#define TOU_DEFAULT_MSS (1500) // max payload size

#define TOU_DEFAULT_RECVWINDOW_SIZE (10)
#define TOU_DEFAULT_SENDWINDOW_SIZE (10)
#define TOU_DEFAULT_RECV_WORKBUFFER_SIZE (TOU_DEFAULT_MSS * 3)
#define TOU_DEFAULT_RECV_INBUFFER_SIZE (TOU_DEFAULT_MSS * 3)
#define TOU_DEFAULT_SEND_WORKBUFFER_SIZE (TOU_DEFAULT_MSS * 3)
#define TOU_DEFAULT_SEND_OUTBUFFER_SIZE (TOU_DEFAULT_MSS * TOU_DEFAULT_SENDWINDOW_SIZE)
#define TOU_DEFAULT_RECV_CTRLBUFFER_SIZE ((TOU_LEN_PKT_ACK + sizeof(ack_id_t) * TOU_PKT_ACK_MAX_ACK_COUNT) * 5)
#define TOU_DEFAULT_ACK_TIMEOUT_MS (5)
#define TOU_DEFAULT_FAST_RETRANSMIT_ACK_COUNT (3)
#define TOU_DEFAULT_RETRANSMIT_METHOD(conn, expired_pkt, dropped_id, ack_stack_count) tou_retransmit_all(conn)
#endif

#ifdef SERVER3
#define TOU_CONFIG_NAME "SERVER_3"
#define TOU_DEFAULT_MSS (1500) // max payload size

#define TOU_DEFAULT_RECVWINDOW_SIZE (10)
#define TOU_DEFAULT_SENDWINDOW_SIZE (60)
#define TOU_DEFAULT_RECV_WORKBUFFER_SIZE (TOU_DEFAULT_MSS * 3)
#define TOU_DEFAULT_RECV_INBUFFER_SIZE (TOU_DEFAULT_MSS * 3)
#define TOU_DEFAULT_SEND_WORKBUFFER_SIZE (TOU_DEFAULT_MSS * 3)
#define TOU_DEFAULT_SEND_OUTBUFFER_SIZE (TOU_DEFAULT_MSS * TOU_DEFAULT_SENDWINDOW_SIZE)
#define TOU_DEFAULT_RECV_CTRLBUFFER_SIZE ((TOU_LEN_PKT_ACK + sizeof(ack_id_t) * TOU_PKT_ACK_MAX_ACK_COUNT) * 5)
#define TOU_DEFAULT_ACK_TIMEOUT_MS (4)
#define TOU_DEFAULT_FAST_RETRANSMIT_ACK_COUNT (3)
#define TOU_DEFAULT_RETRANSMIT_METHOD(conn, expired_pkt, dropped_id, ack_stack_count) tou_retransmit_all(conn)
#endif

#endif
