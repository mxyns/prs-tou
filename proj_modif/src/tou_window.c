...
tou_window* tou_make_window(
    ...
    tou_window* window = (tou_window*) calloc(1, sizeof(tou_window) + window_size * (max_packet_size * sizeof(char) +
                                                                                     sizeof(tou_packet_dtp)));

    char* packet_buffers = (char*) (window + 1); // start of packets buffers
    tou_packet_dtp* packets = (tou_packet_dtp*) (packet_buffers + window_size * max_packet_size *
                                                                  sizeof(char)); // start of packets structs
...
void tou_window_accept_ack_packet_subroutine(
    ...
    if (ack->ack_count == TOU_PKT_ACK_MAX_ACK_COUNT) {
        TOU_DEBUG(printf("[tou][tou_window_accept_ack_packet_subroutine] ack full sending and reset\n"));
        tou_send_ack(conn, ack);
        ack->ack_count = 0; // no need to reset buffer, we'll just read in range [buffer, buffer+ack_count]
    }
    if (!packet->acked) {
        TOU_DEBUG(printf("[tou][tou_window_accept_ack_packet_subroutine] mark id=%d at pos=%d ptr=%p\n",
...
int tou_window_accept(
    ...
    char poperr = 0;
    int popped = 0;
    TOU_DEBUG(printf("[tou][tou_window_accept] while : !ISEMPTY = %d, HEAD!=NULL = %d, VAL!=NULL = %d\n",
    ...
    while (!TOU_SLL_ISEMPTY(window->list) && window->list->head != NULL && window->list->head->val != NULL) {
        ...
        TOU_DEBUG(printf("[tou][tou_window_accept] processing packet %d\n", next_packet->packet_id));

        if (next_packet->packet_id < window->expected) {
            TOU_DEBUG(printf("[tou][tou_window_accept] got unexpected packet, it may be a duplicate\n"));
            next_packet = tou_sll_pop(window->list, &poperr);
            if (poperr || next_packet == NULL) {
                TOU_DEBUG(printf("next_packet poperr packet_id<expected\n"));
                break;
            } // early stop
            ...
            TOU_DEBUG(printf("[tou][tou_window_accept] intermediate packet missing for %d packet\n",
                             next_packet->packet_id));
            ...
            TOU_DEBUG(printf("[tou][tou_window_accept] packet was expected\n"));
            ...
            next_packet = tou_sll_pop(window->list, &poperr);
            if (poperr || next_packet == NULL) {
                TOU_DEBUG(printf("next_packet poperr expected\n"));
                break;
            }
            TOU_DEBUG(printf("[tou][tou_window_accept] packet was popped\n"));
            ...
            if (!packet_inserted) { // if not inserted into stream (lack of space in stream), packet is added back to window
                ...
                tou_sll_insert(window->list,
                               next_packet->packet_id); // no need to modify node value bc it was left intact
                break;
            }
            TOU_DEBUG(printf("[tou][tou_window_accept] packet was written\n"));
    ...
    if (poperr != 0) {
        TOU_DEBUG(printf("[tou][tou_window_accept] error while popping next packet, empty window?\n"));
    }
