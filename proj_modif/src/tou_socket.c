...
tou_socket* tou_make_socket(
    ...
    if (socket_desc < 0) {
        TOU_DEBUG(printf("Error while creating socket\n"));
        return NULL;
    }
    TOU_DEBUG(printf("Socket created successfully\n"));

    int reuse = 1;
    if (setsockopt(socket_desc, SOL_SOCKET, SO_REUSEADDR, (const char*) &reuse, sizeof(reuse)) < 0)
            TOU_DEBUG(perror("setsockopt(SO_REUSEADDR) failed"));

    if (setsockopt(socket_desc, SOL_SOCKET, SO_REUSEPORT, (const char*) &reuse, sizeof(reuse)) < 0)
            TOU_DEBUG(perror("setsockopt(SO_REUSEPORT) failed"));

    sock->fd = socket_desc;
    struct sockaddr_in* server_addr = (struct sockaddr_in*) calloc(1, sizeof(struct sockaddr));
    if (server_addr == NULL) {
        TOU_DEBUG(printf("[tou][make_socket] couldn't allocate memory for sockaddr\n"));
        return NULL;
    }
