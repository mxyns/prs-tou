    ...
    while((size = conn->out->cnt) > 0 && send_list->count < send_list->cap) {
      ...
      // TODO move header creation and pkt struct fill to function in tou_packet.c
       ...
       printf("[tou][tou_send] queued %d bytes\n", popped);
        ...
        char header[TOU_LEN_DTP];
        header[0] = TOU_ID_DTP; // packet_type
        TOU_ARR_INS(header, TOU_HOFFSET_DTP_PACKET_ID, uint32_t, pkt->packet_id); // packet_id
        TOU_ARR_INS(header, TOU_HOFFSET_DTP_PACKET_SIZE, uint32_t, pkt->data_packet_size); // pkt_size

        tou_write_packet(conn->socket, header, TOU_LEN_DTP, payload, popped);
        new_packets++;
    }
